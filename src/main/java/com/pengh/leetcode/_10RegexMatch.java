...
public class _10RegexMatch {
    ...
    public static boolean isMatch(String s, String p) {
        ...
        if(p.length()>=2&&p.charAt(1)=='*'){
            return (matchFirst && isMatch(s.substring(1),p))||isMatch(s,p.substring(2));
        }
        else{
            return matchFirst && isMatch(s.substring(1),p.substring(1));
        }

//        //使用dp[i][j]来表示s[i:]和p[j:]的匹配情况
//        boolean dp[][]=new boolean[s.length()][p.length()];
    ...
    public static void main(String[] args) {
        System.out.println(isMatch("aaabc","a..bc"));
        System.out.println(isMatch("aaabc","a...bc"));
        System.out.println(isMatch("aaabc","a*bc"));
        System.out.println(isMatch("aaabc","a*b."));
    }
}
